...
#   target_density <- exp_sqrt_pdf(x, lambda_sl, beta2) + 0.00001
...
for(i in 1:24) {
  ...
  # Check that dexp is larger than the target density
  ratio_max_x <- cx_1 * dexp(max_x, rate = lambda_solution)/ 
    exp_sqrt_pdf(max_x, lambda_sl = hour_coefs_nat_df$lambda_sl[i], beta2 = hour_coefs_nat_df$sl_sqrt[i])
  
  print(paste0("Ratio between proposal and target at max_x is ", ratio_max_x))

  plot(cx_1 * dexp(x, rate = lambda_solution), type = "l", col = "red")
  lines(exp_sqrt_pdf(x, lambda_sl = hour_coefs_nat_df$lambda_sl[i], beta2 = hour_coefs_nat_df$sl_sqrt[i]))
  # lines(dexp(x, rate = hourly_coefs$rate[i]), col = "blue")
  # lines(cx_1 * dexp(x, rate = lambda_solution), col = "red")
...
Accept-reject function with Cauchy distribution

```{r}

pdf <- function(x, lambda_sl, beta2) {
  exp((lambda_sl * x) + (beta2 * sqrt(x)))
}

# Accept-reject sampling using Cauchy distribution
cauchy_accept_reject <- function(pdf, lambda_sl, beta2, x0, gamma, n, cauchy_scaling) {
  samples <- numeric(0)
  
  while (length(samples) < n) {
    x_proposal <- rcauchy(1, location = x0, scale = gamma)
    if (x_proposal <= 0) next  # Skip non-positive values
    if (x_proposal > max_x) next  # Skip values greater than the 99th quantile of step lengths
    accept_prob <- pdf(x_proposal, lambda_sl, beta2) / (cauchy_scaling * dcauchy(x_proposal, location = x0, scale = gamma))
...
plot_densities <- function(lambda_sl, beta2, x0, gamma, max_x, cauchy_scaling) {
  x_values <- seq(0, max_x, length.out = 1000)
  target_densities <- sapply(x_values, function(x) exp_sqrt_pdf(x, lambda_sl, beta2))
  cauchy_densities <- cauchy_scaling * dcauchy(x_values, location = x0, scale = gamma)

  data <- data.frame(x = x_values, 
                     Target = target_densities / sum(target_densities), 
                     Cauchy = cauchy_densities ) # / sum(cauchy_densities))
                     # Target = target_densities, 
                     # Cauchy = cauchy_densities) 

  ggplot(data) +
    geom_line(aes(x = x, y = Target, colour = "Target")) +
    geom_line(aes(x = x, y = Cauchy, colour = "Cauchy")) +
    scale_colour_manual(values = c("Target" = "blue", "Cauchy" = "red")) +
    ggtitle(paste0("Density Comparison - Hour ", i)) +
    theme_bw()
}
...
plot_samples <- function(samples) {
  ggplot(data.frame(x = samples), aes(x)) +
    ...
    ggtitle(paste0("Histogram of Sampled Values - Hour ", i)) +
    theme_bw()
}

```

Running the accept-reject function
...
for(i in 1:24) {
  ...
  gamma <- 10
  n_samples <- 1e4
  max_x <- max_x  # Adjust as needed
  cauchy_scaling <- pdf(max_x, lambda_sl, beta2) / dcauchy(max_x, location = x0, scale = gamma)
  
  tic()
  # Generate samples
  samples <- cauchy_accept_reject(exp_sqrt_pdf, lambda_sl, beta2, x0, gamma, n_samples, cauchy_scaling)
  toc()
  
  # Plot densities and samples
  # png(paste0("outputs/plots/accept_reject_cauchy/cauchy_target_densities_", i, ".png"))
  print(plot_densities(lambda_sl, beta2, x0, gamma, max_x, cauchy_scaling))
  # dev.off()
  
  # png(paste0("outputs/plots/accept_reject_cauchy/histogram_samples_", i, ".png"))
  print(plot_samples(samples))
  # dev.off()
  
  print(paste0("Mean of samples is ", mean(samples)))
  print(paste0("Mean of samples is ", median(samples)))
