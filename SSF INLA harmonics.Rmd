...
# unique(buffalo_CLR_year$id)[1:3]

n_indvs <- 2

# buffalo_inla <- buffalo_CLR_year_harmonics %>% 
buffalo_inla <- buffalo_CLR_year_3pts %>% 
  filter(id %in% unique(buffalo_CLR_year$id)[1:n_indvs]) %>% 
...
formula.random <- y ~ -1 + 
  ...
  f(id3, ndvi_c1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  # f(id5, ndvi_c2, values=1:n_indvs,model="iid",
  #   hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +

  f(id4, canopy_scaled, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +

  # f(id4, canopy_2, values=1:n_indvs,model="iid",
  #   hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05))))
  
  f(id5, canopy_s1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  # f(id8, canopy_s2, values=1:n_indvs,model="iid",
  #   hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id6, canopy_c1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  # f(id10, canopy_c2, values=1:n_indvs,model="iid",
  #   hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id7, sl, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id8, sl_s1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id9, sl_c1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id10, log_sl, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id11, log_sl_s1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id12, log_sl_c1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id13, cos_ta, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id14, cos_ta_s1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05)))) +
  
  f(id15, cos_ta_c1, values=1:n_indvs,model="iid",
    hyper=list(theta=list(initial=log(1),fixed=F,prior="pc.prec",param=c(3,0.05))))
...
tic()
inla_harmonics <- inla(formula.random, 
              family = "Poisson", 
              data = buffalo_inla,
              # verbose = T,
              # control.inla = list(int.strategy = "eb"),
...
summary(inla_harmonics)
# m_all[[i]]$dic[[1]]
inla_harmonics$summary.fixed
bri.hyperpar.summary(inla_harmonics)

bri.fixed.plot(inla_harmonics)
bri.hyperpar.plot(inla_harmonics)

# Posterior of individuals intercepts for ndvi
ggplot() + 
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id1$index.1), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id1$index.2), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id1$index.3), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id1$index.4), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id1$index.5), aes(x = x, y = y))

# Posterior of individuals intercepts for ndvi^2
ggplot() + 
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id2$index.1), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id2$index.2), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id2$index.3), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id2$index.4), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id2$index.5), aes(x = x, y = y))

# Posterior of individuals intercepts for canopy
ggplot() + 
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id3$index.1), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id3$index.2), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id3$index.3), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id3$index.4), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id3$index.5), aes(x = x, y = y))

# Posterior of individuals intercepts for canopy^2
ggplot() + 
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id4$index.1), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id4$index.2), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id4$index.3), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id4$index.4), aes(x = x, y = y)) +
  geom_line(data = as.data.frame(inla_harmonics$marginals.random$id4$index.5), aes(x = x, y = y))
