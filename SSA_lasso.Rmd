...
Formula with two pairs of harmonics ~ ndvi + ndvi2 + canopy + canopy2 + herby

```{r}

sum(is.na(buffalo_CLR_year_harmonics$elev_log))
min(buffalo_CLR_year_harmonics$DEM_H_end)
...
Formula with two pairs of harmonics ~ ndvi + ndvi2 + canopy + canopy2 + herby + elev_scaled
...
buffalo_lasso <- buffalo_data %>% mutate(
  
  ndvi = scale(ndvi_temporal),
  ndvi_s1 = scale(ndvi_temporal * yday_s1),
  ndvi_s2 = scale(ndvi_temporal * yday_s2),
  ndvi_s3 = scale(ndvi_temporal * yday_s3),
  ndvi_c1 = scale(ndvi_temporal * yday_c1),
  ndvi_c2 = scale(ndvi_temporal * yday_c2),
  ndvi_c3 = scale(ndvi_temporal * yday_c3),
  
  ndvi_sq = scale(ndvi_temporal ^ 2),
  ndvi_sq_s1 = scale((ndvi_temporal ^ 2) * yday_s1),
  ndvi_sq_s2 = scale((ndvi_temporal ^ 2) * yday_s2),
  ndvi_sq_s3 = scale((ndvi_temporal ^ 2) * yday_s3),
  ndvi_sq_c1 = scale((ndvi_temporal ^ 2) * yday_c1),
  ndvi_sq_c2 = scale((ndvi_temporal ^ 2) * yday_c2),
  ndvi_sq_c3 = scale((ndvi_temporal ^ 2) * yday_c3),
  
  canopy = canopy_scaled,
  canopy_s1 = canopy_scaled * yday_s1,
  canopy_s2 = canopy_scaled * yday_s2,
  canopy_s3 = canopy_scaled * yday_s3,
  canopy_c1 = canopy_scaled * yday_c1,
  canopy_c2 = canopy_scaled * yday_c2,
  canopy_c3 = canopy_scaled * yday_c3,
  
  canopy_sq = canopy_2,
  canopy_sq_s1 = canopy_2 * yday_s1,
  canopy_sq_s2 = canopy_2 * yday_s2,
  canopy_sq_s3 = canopy_2 * yday_s3,
  canopy_sq_c1 = canopy_2 * yday_c1,
  canopy_sq_c2 = canopy_2 * yday_c2,
  canopy_sq_c3 = canopy_2 * yday_c3,
  
  herby = herby_scaled,
  herby_s1 = herby_scaled * yday_s1,
  herby_s2 = herby_scaled * yday_s2,
  herby_s3 = herby_scaled * yday_s3,
  herby_c1 = herby_scaled * yday_c1,
  herby_c2 = herby_scaled * yday_c2,
  herby_c3 = herby_scaled * yday_c3,
  
  # elev = elev_scaled,
  # elev_s1 = elev_scaled * yday_s1,
  # elev_s2 = elev_scaled * yday_s2,
  # elev_s3 = elev_scaled * yday_s3,
  # elev_c1 = elev_scaled * yday_c1,
  # elev_c2 = elev_scaled * yday_c2,
  # elev_c3 = elev_scaled * yday_c3,
  # 
  # elev_2 = elev_2,
  # elev_2_s1 = elev_2 * yday_s1,
  # elev_2_s2 = elev_2 * yday_s2,
  # elev_2_s3 = elev_2 * yday_s3,
  # elev_2_c1 = elev_2 * yday_c1,
  # elev_2_c2 = elev_2 * yday_c2,
  # elev_2_c3 = elev_2 * yday_c3,
...
buffalo_lasso_scaled <- buffalo_data %>% mutate(
  ...
  canopy = scale(canopy_cover),
  canopy_s1 = scale(canopy_cover * yday_s1),
  canopy_s2 = scale(canopy_cover * yday_s2),
  canopy_s3 = scale(canopy_cover * yday_s3),
  canopy_c1 = scale(canopy_cover * yday_c1),
  canopy_c2 = scale(canopy_cover * yday_c2),
  canopy_c3 = scale(canopy_cover * yday_c3),
  
  canopy_sq = scale(canopy_cover ^ 2),
  canopy_sq_s1 = scale((canopy_cover ^ 2) * yday_s1),
  canopy_sq_s2 = scale((canopy_cover ^ 2) * yday_s2),
  canopy_sq_s3 = scale((canopy_cover ^ 2) * yday_s3),
  canopy_sq_c1 = scale((canopy_cover ^ 2) * yday_c1),
  canopy_sq_c2 = scale((canopy_cover ^ 2) * yday_c2),
  canopy_sq_c3 = scale((canopy_cover ^ 2) * yday_c3),
  
  herby = scale(veg_herby),
  herby_s1 = scale(veg_herby * yday_s1),
  herby_s2 = scale(veg_herby * yday_s2),
  herby_s3 = scale(veg_herby * yday_s3),
  herby_c1 = scale(veg_herby * yday_c1),
  herby_c2 = scale(veg_herby * yday_c2),
  herby_c3 = scale(veg_herby * yday_c3),
  ...
  step_l = scale(sl),
  step_l_s1 = scale(sl * yday_s1),
  step_l_s2 = scale(sl * yday_s2),
  step_l_s3 = scale(sl * yday_s3),
  step_l_c1 = scale(sl * yday_c1),
  step_l_c2 = scale(sl * yday_c2),
  step_l_c3 = scale(sl * yday_c3),

  log_step_l = scale(log_sl),
  log_step_l_s1 = scale(log_sl * yday_s1),
  log_step_l_s2 = scale(log_sl * yday_s2),
  log_step_l_s3 = scale(log_sl * yday_s3),
  log_step_l_c1 = scale(log_sl * yday_c1),
  log_step_l_c2 = scale(log_sl * yday_c2),
  log_step_l_c3 = scale(log_sl * yday_c3),

  cos_turn_a = scale(cos_ta),
  cos_turn_a_s1 = scale(cos_ta * yday_s1),
  cos_turn_a_s2 = scale(cos_ta * yday_s2),
  cos_turn_a_s3 = scale(cos_ta * yday_s3),
  cos_turn_a_c1 = scale(cos_ta * yday_c1),
  cos_turn_a_c2 = scale(cos_ta * yday_c2),
  cos_turn_a_c3 = scale(cos_ta * yday_c3),
...
# fit the conditional logistic model
tic()
cl_buffalo = clogitL1(y=y, x=X, strata, 
                       # numLambda = 100,
                       # minLambdaRatio = 1e-8
                      alpha = 1
                       )
...
# cross validation
tic()
clcv_buffalo = cv.clogitL1(cl_buffalo)
toc()
plot(clcv_buffalo)
...
# Extract centering and scaling attributes from each column
column_attributes <- as.data.frame(lapply(buffalo_lasso_scaled, function(x) {
  center <- attr(x, "scaled:center")  # Extract the centering attribute
  scale <- attr(x, "scaled:scale")  # Extract the scaling attribute
  
  # Return the extracted attributes as a named list
...
buffalo_lasso_unscaled <- buffalo_data %>% mutate(
  ...
  canopy_sq = canopy_scaled ^ 2,
  canopy_sq_s1 = (canopy_scaled ^ 2) * yday_s1,
  canopy_sq_s2 = (canopy_scaled ^ 2) * yday_s2,
  canopy_sq_s3 = (canopy_scaled ^ 2) * yday_s3,
  canopy_sq_c1 = (canopy_scaled ^ 2) * yday_c1,
  canopy_sq_c2 = (canopy_scaled ^ 2) * yday_c2,
  canopy_sq_c3 = (canopy_scaled ^ 2) * yday_c3,
...
# coefs[which(coefs == "ndvi_s1")]
# ndvi_s2_harm <- as.numeric(coef_betas_CV1se[which(coefs == "ndvi_s2")] %*% yday_harmonics_df$yday_s2)
# plot(yday, ndvi_s2_harm)

# ndvi_harm <- as.numeric(coefs_df %>% dplyr::filter(grepl("ndvi", coefs) & !grepl("sq", coefs)) %>% 
#                           dplyr::select(beta_CV) %>% unlist() %>% as.numeric() 
#                         %*% t(as.matrix(yday_harmonics_df)))
# 
# ndvi_harm1se <- as.numeric(coefs_df %>% dplyr::filter(grepl("ndvi", coefs) & !grepl("sq", coefs)) %>% 
#                           dplyr::select(beta_CV1se) %>% unlist() %>% as.numeric() 
#                         %*% t(as.matrix(yday_harmonics_df)))
# 
# plot(yday, ndvi_harm)
# plot(yday, ndvi_harm1se)
