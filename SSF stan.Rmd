...
buffalo_CLR_year %>% mutate(ndvi_s1 = ndvi_scaled[,1] * yday_s1,
...
buffalo_subset <- buffalo_CLR_year %>%
  ...
  dplyr::select(id, id_num, ndvi_scaled, herby_scaled, canopy_scaled, y, step_id, step_aligned, sl, log_sl, cos_ta) %>% 
  drop_na(cos_ta)
...
tic()
res_stan_mvmt <- sampling(stan_mod, stan_dat, cores = 4, chains = 1, iter = 100)
toc()
