...
formula_clr <- y ~ 
               ...
               ndvi_scaled +
               month_s1:ndvi_scaled +
               month_s2:ndvi_scaled +
               month_c1:ndvi_scaled +
               month_c2:ndvi_scaled +
  
               I(ndvi_scaled^2) +
               month_s1:I(ndvi_scaled^2) +
               month_s2:I(ndvi_scaled^2) +
               month_c1:I(ndvi_scaled^2) +
               month_c2:I(ndvi_scaled^2) +
...
# clogit_coefs <- clogit_coefs %>% mutate(weight = 1/se^2) # does the same as above
...
y_ndvi_month <- map_dbl(x_month,
             ...
             clogit_coef_df$value[which(clogit_coef_df$coef == "ndvi_scaled:month_s1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "ndvi_scaled:month_s2")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "ndvi_scaled:month_c1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "ndvi_scaled:month_c2")])
...
y_canopy <- map_dbl(x_month,
             ...
             clogit_coef_df$value[which(clogit_coef_df$coef == "canopy_scaled:month_s1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "canopy_scaled:month_s2")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "canopy_scaled:month_c1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "canopy_scaled:month_c2")])

y_sl <- map_dbl(x_month,
             y_month,
             clogit_coef_df$value[which(clogit_coef_df$coef == "sl")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "sl:month_s1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "sl:month_s2")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "sl:month_c1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "sl:month_c2")])

y_log_sl <- map_dbl(x_month,
             y_month,
             clogit_coef_df$value[which(clogit_coef_df$coef == "log_sl")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "log_sl:month_s1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "log_sl:month_s2")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "log_sl:month_c1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "log_sl:month_c2")])

y_cos_ta <- map_dbl(x_month,
             y_month,
             clogit_coef_df$value[which(clogit_coef_df$coef == "cos_ta")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "cos_ta:month_s1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "cos_ta:month_s2")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "cos_ta:month_c1")],
             clogit_coef_df$value[which(clogit_coef_df$coef == "cos_ta:month_c2")])

return(data.frame("NDVI" = y_ndvi_month, 
                  # "wofs05" = y_wofs05_month, 
                  # "wofs05_Q" = y_wofs05_Q_month, 
                  "Slope" = y_slope_month, 
                  "Herbaceous_vegetation" = y_herby_month, 
...
# combining
harmonic_coefs_df <- cbind("id" = rep(c(buffalo_ids, "Weighted mean", "Mean"), each = 121), 
                           "month" = x_month, harmonic_coef_vals_w_mean)
...
for(i in 1:length(coefs)) {
  ...
  geom_line(data = harmonic_coefs %>% 
              ...
              filter(coef == coefs[i]), 
            aes(x = month, y = value, group = id), colour = "grey") + # , colour = factor(id)
    geom_line(data = harmonic_coefs %>% filter(id %in% unique(harmonic_coefs$id)[11:12]) %>% 
                filter(coef == coefs[i]), aes(x = month, y = value, colour = factor(id))) +
    geom_hline(yintercept = 0, linetype = "dashed") +
...
monthly_coefs <- data.frame("date" = dates,
                         ...
                         "veg_herby" = harmonic_weighted_mean$Herbaceous_vegetation[seq(1, 111, 10)],
                         "canopy" = harmonic_weighted_mean$Canopy_cover[seq(1, 111, 10)],
                         "shape" = harmonic_weighted_mean$shape[seq(1, 111, 10)],
                         "scale" = harmonic_weighted_mean$scale[seq(1, 111, 10)],
                         "kappa" = harmonic_weighted_mean$kappa[seq(1, 111, 10)])

write_csv(monthly_coefs, "outputs/monthly_CLR_coefs_wide_with_movement_params_20230210.csv") # weighted mean
