...
packages <- c("amt", "lubridate", "mgcv", "survival", "terra", "raster", "tictoc", 
              ...
              "sf", "ggpubr", "circular", "glmmTMB", "beepr")

walk(packages, require, character.only = T)

```
Import buffalo telemetry data using data prep file
...
# buffalo_data <- buffalo_month_subset

# using the full dataset
# buffalo_data <- buffalo_CLR_year

# using the full dataset with harmonics precomputed (using the stan script)
buffalo_data <- buffalo_subset %>% mutate(sl_scaled = scale(sl)) # from the SSF stan code
...
glmmTMB_model <- glmmTMB(y ~ -1 +
                         ...
                         start = list(theta = c(log(1e3), rep(0,5)))
                         )
toc()
...
canopy_lin <- glmmTMB_model$fit$par[4]
canopy_quad <- glmmTMB_model$fit$par[5]
...
formula_glmmtmb <- y ~ -1 +
                           # ndvi_scaled +
                           #  I(ndvi_scaled^2) +
                           poly(ndvi_scaled, degree = 1, raw = TRUE) +
                           poly(ndvi_s1, degree = 1, raw = TRUE) +
                           poly(ndvi_s2, degree = 1, raw = TRUE) +
                           poly(ndvi_c1, degree = 1, raw = TRUE) +
                           poly(ndvi_c2, degree = 1, raw = TRUE) +
                            # slope_scaled +
                            # I(slope_scaled^2) +
                             # herby_scaled +
                             # poly(veg_herby, degree = 1, raw = TRUE) +
                            # elev_log +
                            # I(elev_log^2) +
                             # poly(water_dist, degree = 2, raw = TRUE) +
                             # poly(water_dist_log, degree = 2, raw = TRUE) +
                             # water_dist +
                            # water_dist_log +
                            # I(water_dist_log^2) +
                            # canopy_scaled +
                            # I(canopy_scaled^2) +
                           poly(canopy_scaled, degree = 1, raw = TRUE) +
                           poly(canopy_s1, degree = 1, raw = TRUE) +
                           poly(canopy_s2, degree = 1, raw = TRUE) +
                           poly(canopy_c1, degree = 1, raw = TRUE) +
                           poly(canopy_c2, degree = 1, raw = TRUE) +
                            ...
                            # poly(sl, degree = 1, raw = TRUE) +
                            # poly(log_sl, degree = 1, raw = TRUE) +
                            #  poly(cos_ta, degree = 1, raw = TRUE) +
                             
                             (1 | step_id) +
                           # (0 + ndvi_scaled | id) +
                             (0 + poly(ndvi_scaled, degree = 1, raw = TRUE) | id) +
                           (0 + poly(ndvi_s1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(ndvi_s2, degree = 1, raw = TRUE) | id) +
                           (0 + poly(ndvi_c1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(ndvi_c2, degree = 1, raw = TRUE) | id) +
                           # (0 + slope_scaled | id) +
                             # (0 + poly(veg_herby, degree = 1, raw = TRUE) | id) +
                             # (0 + poly(water_dist, degree = 1, raw = TRUE) | id) +
                           # (0 + canopy_scaled | id)
                             (0 + poly(canopy_scaled, degree = 1, raw = TRUE) | id) +
                           (0 + poly(canopy_s1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(canopy_s2, degree = 1, raw = TRUE) | id) +
                           (0 + poly(canopy_c1, degree = 1, raw = TRUE)| id) +
                           (0 + poly(canopy_c2, degree = 1, raw = TRUE) | id) +
  
                            (0 + poly(sl, degree = 1, raw = TRUE) | id) +
                           (0 + poly(sl_s1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(sl_s2, degree = 1, raw = TRUE) | id) +
                           (0 + poly(sl_c1, degree = 1, raw = TRUE)| id) +
                           (0 + poly(sl_c2, degree = 1, raw = TRUE) | id) +
  
                            (0 + poly(log_sl, degree = 1, raw = TRUE) | id) +
                           (0 + poly(log_sl_s1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(log_sl_s2, degree = 1, raw = TRUE) | id) +
                           (0 + poly(log_sl_c1, degree = 1, raw = TRUE)| id) +
                           (0 + poly(log_sl_c2, degree = 1, raw = TRUE) | id) +
  
                            (0 + poly(cos_ta, degree = 1, raw = TRUE) | id) +
                           (0 + poly(cos_ta_s1, degree = 1, raw = TRUE) | id) +
                           (0 + poly(cos_ta_s2, degree = 1, raw = TRUE) | id) +
                           (0 + poly(cos_ta_c1, degree = 1, raw = TRUE)| id) +
                           (0 + poly(cos_ta_c2, degree = 1, raw = TRUE) | id)

```

With harmonics and quadratics
...
formula_glmmtmb <- y ~ -1 +
                           ...
                           #  I(ndvi_scaled^2) +
                           poly(ndvi_scaled, degree = 2, raw = FALSE) +
                           poly(ndvi_s1, degree = 2, raw = FALSE) +
                           poly(ndvi_s2, degree = 2, raw = FALSE) +
                           poly(ndvi_c1, degree = 2, raw = FALSE) +
                           poly(ndvi_c2, degree = 2, raw = FALSE) +
                            # slope_scaled +
                            # I(slope_scaled^2) +
                             # herby_scaled +
                             # poly(veg_herby, degree = 2, raw = FALSE) +
                            # elev_log +
                            # I(elev_log^2) +
                             # poly(water_dist, degree = 2, raw = TRUE) +
                             # poly(water_dist_log, degree = 2, raw = TRUE) +
                            ...
                            # I(canopy_scaled^2) +
                           poly(canopy_scaled, degree = 2, raw = FALSE) +
                           poly(canopy_s1, degree = 2, raw = FALSE) +
                           poly(canopy_s2, degree = 2, raw = FALSE) +
                           poly(canopy_c1, degree = 2, raw = FALSE) +
                           poly(canopy_c2, degree = 2, raw = FALSE) +
                            ...
                            # poly(sl, degree = 2, raw = FALSE) +
                            # poly(log_sl, degree = 2, raw = FALSE) +
                            #  poly(cos_ta, degree = 2, raw = FALSE) +
                             
                             (1 | step_id) +
                           # (0 + ndvi_scaled | id) +
                           (0 + poly(ndvi_scaled, degree = 2, raw = FALSE) | id) +
                           (0 + poly(ndvi_s1, degree = 2, raw = FALSE) | id) +
                           (0 + poly(ndvi_s2, degree = 2, raw = FALSE) | id) +
                           (0 + poly(ndvi_c1, degree = 2, raw = FALSE) | id) +
                           (0 + poly(ndvi_c2, degree = 2, raw = FALSE) | id) +
                           # (0 + slope_scaled | id) +
                             # (0 + poly(veg_herby, degree = 2, raw = FALSE) | id) +
                             # (0 + poly(water_dist, degree = 2, raw = FALSE) | id) +
                           # (0 + canopy_scaled | id)
                           (0 + poly(canopy_scaled, degree = 2, raw = FALSE) | id) +
                           (0 + poly(canopy_s1, degree = 2, raw = FALSE) | id) +
                           (0 + poly(canopy_s2, degree = 2, raw = FALSE) | id) +
                           (0 + poly(canopy_c1, degree = 2, raw = FALSE) | id) +
                           (0 + poly(canopy_c2, degree = 2, raw = FALSE) | id) +
...
tic()
glmmTMB_model <- glmmTMB(formula = formula_glmmtmb,
                         data = buffalo_CLR_year_harmonics, 
                         family = poisson,
                         map = list(theta = factor(c(NA, 1:45))),
                         start = list(theta = c(log(1e3), rep(0,45))) # 
                         )
toc()
